...
/**
 * Validate workflow structure and return detailed errors
 *
 * Checks for:
 ...
 *
 * @param workflowData - Workflow data to validate
...
export function validateWorkflow(workflowData: WorkflowData): {
  ...
  const actionNodes = workflowData.nodes.filter(
    (node) => node.type === "action",
  );
  if (actionNodes.length === 0) {
    errors.push("Workflow must contain at least one action node");
  }
