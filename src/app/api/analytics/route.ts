...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalViews = marketplaceItems.reduce((sum, item) => sum + (item.Views || 0), 0);
    const totalDownloads = marketplaceItems.reduce((sum, item) => sum + (item.Downloads || 0), 0);
    const totalLikes = marketplaceItems.reduce((sum, item) => sum + (item.Likes || 0), 0);
...
function generateChartData(
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    // Tính toán dữ liệu ngẫu nhiên dựa trên tổng
